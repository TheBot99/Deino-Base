            ...
            ZStack {
                ...
                GeometryReader { geometry in
                    VStack {
                        ...
                        SearchBar(text: $searchValue, selectedFilter: $selectedFilter, placeholder: "Search", initialHeight: initialHeight)
                                                    .frame(height: (initialHeight == 0 ? geometry.size.height : initialHeight) * 0.1)
                        
                        PokemonCardList(width: geometry.size.width * 0.9, height: (initialHeight == 0 ? geometry.size.height : initialHeight) * 0.125, searchValue: searchValue, filter: selectedFilter)
